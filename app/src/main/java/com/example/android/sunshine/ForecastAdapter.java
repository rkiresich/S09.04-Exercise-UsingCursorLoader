...
//      TODO (7) Generate a weather summary with the date, description, high and low
        ...
        String formattedDate = SunshineDateUtils.getFriendlyDateString(mContext, date, false);

        double highTemp = mCursor.getDouble(MainActivity.INDEX_WEATHER_HIGH_TEMP);
        double lowTemp = mCursor.getDouble(MainActivity.INDEX_WEATHER_LOW_TEMP);
        String formatHiLo = SunshineWeatherUtils.formatHighLows(mContext, highTemp, lowTemp);

        int conditionID = mCursor.getInt(MainActivity.INDEX_WEATHER_CONDITION_ID);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext,conditionID);

        String weatherSummary = formattedDate + " - " + description + " --> " + formatHiLo;
